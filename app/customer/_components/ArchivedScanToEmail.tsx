...
const ArchivedScanToEmail = ({ entries, cid }: Props) => {
  ...
  const handleRestore = async (id: string) => {
    setPendingId(id)
    try {
      await axios.post(`/api/customers/${cid}/scan-to-email/${id}/restore`)
      router.refresh()
    } finally {
      setPendingId(null)
    }
  ...
  return (
    <section className="compo">
      <h2 className="text-xl font-semibold mb-3 text-gray-500">Archived Scan To Email</h2>
...
export default ArchivedScanToEmail
