...
interface Props {
  setups: SolutionSetup[]
  cid: string
}

const ArchivedSolutionSetups = ({ setups, cid }: Props) => {
  ...
  const handleRestore = async (id: string) => {
    setPendingId(id)
    try {
      await axios.post(`/api/customers/${cid}/solution-setup/${id}/restore`)
      router.refresh()
    } finally {
      setPendingId(null)
    }
  ...
  return (
    <section className="compo">
      <h2 className="text-xl font-semibold mb-3 text-gray-500">Archived Solution Setups</h2>
...
export default ArchivedSolutionSetups
